...
export class PromoAdminService {
  ...
  public promoListSubject$: BehaviorSubject<Promo[]> = new BehaviorSubject<
    Promo[]
  >([]);
  ...
  updatePromoList(): void {
    this.DBAccessor.getAllPromos$().subscribe((promos) =>
      this.promoListSubject$.next(promos),
    );
  }

  addPromo(promo: Promo): void {
    this.DBAccessor.addPromo$(promo).subscribe((newPromo) => {
      this.updatePromoList();
    });
  }

  deletePromo(promo: Promo): void {
    this.DBAccessor.deletePromo(promo).subscribe((newPromo) => {
      this.updatePromoList();
    });
  }
}
